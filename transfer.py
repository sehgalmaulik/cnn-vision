...
num_ftrs_pretrained = model_pretrained.fc.in_features
model_pretrained.fc = nn.Linear(num_ftrs_pretrained, len(class_names))
...
start_time_pretrained = timer()
history_pretrained = train(model_pretrained, train_loader_simple, test_loader_simple, criterion, optimizer_pretrained, NUM_EPOCHS, device)
end_time_pretrained = timer()
print(f"Fine-tuning time with pre-trained weights: {end_time_pretrained - start_time_pretrained:.2f}s")


model_scratch = models.resnet34(pretrained=False)
num_ftrs_scratch = model_scratch.fc.in_features
model_scratch.fc = nn.Linear(num_ftrs_scratch, len(class_names))
model_scratch = model_scratch.to(device)
optimizer_scratch = torch.optim.Adam(model_scratch.parameters(), lr=LEARNING_RATE)


print("Training model from scratch")
start_time_scratch = timer()
history_scratch = train(model_scratch, train_loader_simple, test_loader_simple, criterion, optimizer_scratch, NUM_EPOCHS, device)
end_time_scratch = timer()
print(f"Training time from scratch: {end_time_scratch - start_time_scratch:.2f}s")


plt.plot(history_pretrained["test_acc"], label="Pre-trained")
plt.plot(history_scratch["test_acc"], label="From scratch")
plt.xlabel("Epochs")
plt.ylabel("Test Accuracy")
plt.legend()
plt.title("Test Accuracies for ResNet34 Model")
plt.show()
